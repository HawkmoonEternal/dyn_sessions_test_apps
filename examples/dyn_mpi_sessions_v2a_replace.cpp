...
const unsigned long long PROBLEM_SIZE = 1000000000;
...
int main(int argc, char* argv[])
{
    ...
    MPI_Info_free(&info);
    printf("create comm from pset %s\n", main_pset);
        ...
        if(check_rc && ++iters_since_last_change >= rc_frequency){
            ...
            if(MPI_PSETOP_NULL == rc_type){
                ...
                if(primary_proc){
                    ...
                    MPI_Info_free(&info);
                    printf("setop returned op %d\n", op);

                    if(MPI_PSETOP_NULL != op){
                        /* Publish the name of the new main PSet on the delta Pset */
                        MPI_Info_create(&info);
                        MPI_Info_set(info, "next_main_pset", output_psets[2]);
                        MPI_Session_set_pset_data(session_handle, output_psets[1], info);
                        MPI_Info_free(&info);
                    }
            ...
            printf(" ...is: %d %s vs %s\n", rc_type, delta_pset, output_psets[0]);
            ...
            MPI_Session_get_pset_data (session_handle, main_pset, output_psets[1], (char **) &keys[0], 1, true, &info);
            MPI_Info_get(info, "next_main_pset", MPI_MAX_PSET_NAME_LEN, main_pset, &flag); 
            ...
            if(terminate){
                printf("Rank %d: need to terminate\n", rank);
                break;
            }
            sleep(3);
            /* Disconnect from the old communicator */
            MPI_Comm_disconnect(&comm);
            printf("Rank %d: create comm from pset %s\n", rank, main_pset);
