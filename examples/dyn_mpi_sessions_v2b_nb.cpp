...
int main(int argc, char* argv[])
{
    ...
    if(MPI_SUCCESS != (rc = MPI_Session_get_pset_data_nb (session, mpi_world_pset, mpi_world_pset, (char **) keys, 2, true, &info, &request))){
    ...
    if(flag && 0 == strcmp(boolean_string, "True")){
        ...
        if(MPI_SUCCESS != (rc = MPI_Session_get_pset_data_nb (session, mpi_world_pset, mpi_world_pset, (char **) &keys[2], 1, true, &info, &request))){
            printf("MPI_Session_get_pset_data returned error %d\n", rc);
            MPI_Session_finalize(&session);
            return rc;
        }
        ...
        MPI_Info_free(&info);
        if(MPI_SUCCESS != (rc = MPI_Session_get_pset_data_nb (session, mpi_world_pset, main_pset, (char **) &keys[1], 1, true, &info, &request))){
            printf("MPI_Session_get_pset_data returned error %d\n", rc);
            MPI_Session_finalize(&session);
            return rc;
        }
    ...
    while(cur_iter++ < ITER_MAX){
        ...
        if(check_rc && ++iters_since_last_change >= rc_frequency){
            ...
            if(MPI_SUCCESS != (rc = MPI_Session_dyn_v2b_query_psetop_nb(session, main_pset, main_pset, &rc_handle, &request))){
            ...
            if(MPI_PSETOP_NULL == rc_type){
                ...
                if(primary_proc){
                    ...
                    if(MPI_SUCCESS != (rc = MPI_Session_dyn_v2b_psetop_nb(session, rc_handle, &request))){
            ...
            if(MPI_SUCCESS != (rc = MPI_Session_get_pset_data_nb (session, main_pset, new_delta_pset, (char **) &keys[1], 3, true, &info, &request))){
                printf("MPI_Session_get_pset_data returned error %d\n", rc);
                MPI_Session_finalize(&session);
                return rc;
            }
            ...
            if(MPI_SUCCESS != (rc = MPI_Session_get_pset_data_nb (session, MPI_PSETOP_SUB == rc_type ? main_pset : old_main_pset, main_pset, (char **) &keys[1], 1, true, &info, &request))){
                printf("MPI_Session_get_pset_data returned error %d\n", rc);
                MPI_Session_finalize(&session);
                return rc;
            }
